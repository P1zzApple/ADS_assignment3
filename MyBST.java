...
public class MyBST<K extends Comparable<K>, V> {
    ...
    public Iterable<K> iterator(K l, K h) {
    ...
    private void iterator(Node root, Queue<K> q, K l, K h) {
        int l_comp = l.compareTo(root.key);
        int h_comp = h.compareTo(root.key);
        if (l_comp < 0) {
            iterator(root.left, q, l, h);
        }
        if (l_comp <= 0 && h_comp >= 0) {
            q.add(root.key);
        }
        if (h_comp > 0) {
            iterator(root.right, q, l, h);
        }
    }
}
